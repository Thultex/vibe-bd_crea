...
OpenMoji-Grafiken werden zur Laufzeit geladen und sind in Emotronic gegenüber plattformabhängigen Emoji-Zeichen zu bevorzugen, wo Motiv und Darstellung sinnvoll passen. Der Controller `1F3AE` erscheint farbig am direkten Simon-Einstieg. Nur im ausgeschalteten Zustand erscheinen die schwarz-weiße Videokassette `1F4FC` auf der `R`-Taste und das schwarz-weiße `1F50D` auf dem Aus-Knopf; eingeschaltet werden wieder `R` und das normale Power-Zeichen gezeigt. Die Score-Vorschau verwendet den Pokal `1F3C6`. Text- und ASCII-Fallbacks halten die Anwendung auch bei fehlgeschlagenen Grafik- oder Audiozugriffen bedienbar.
