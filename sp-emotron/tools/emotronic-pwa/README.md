...
Ohne Modusfragment startet Emotronic wie `#on`. `#slow=…` ist der langsame Tag eines vollständigen Replay-Links: Sein Datenteil ist identisch zu `#replay=…`, sodass man zwischen beiden Tempi durch Ersetzen des Fragmentnamens umschalten kann.
...
| `Emotronic-v2.28.html` | Versionierte Kopie der Hauptanwendung |
...
Emotronic speichert geteilte Daten ausschließlich im URL-Fragment. Die Namen bleiben ausgeschrieben: `#share=` für ein Gefühl, `#replay=` für Replays, `#slow=` für langsame Replays und `#score=` für Game-over-Scores. Jeder Gefühlszustand benötigt im Datenteil nur ein Base36-Zeichen; Intensität und Kombination sind darin bereits codiert. Alte Base64url-JSON-Datenteile werden unter denselben Namen weiterhin gelesen. Dadurch werden die Daten nicht automatisch an einen Server übertragen.
...
- Game-over-Links verwenden `#score=…`, normale Replay-Links `#replay=…`, langsame Replay-Links mit identischen Daten `#slow=…` und Gefühlslinks `#share=…`. Die neuen Datenteile sind kompakt; alle bisherigen langen Datenteile werden weiterhin gelesen.
...
- Die aktuelle Cache-Version lautet `emotronic-v128`.
